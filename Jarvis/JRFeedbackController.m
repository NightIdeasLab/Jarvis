...
+ (void)showFeedbackWithBugDetails:(NSString *)details {
    ...
    if (!showFeedbackWindow) {
        NSInteger alertResult = [[NSAlert alertWithMessageText:NSLocalizedStringFromTable(@"Feedback Host Not Reachable", @"JRFeedbackProvider", nil) defaultButton:NSLocalizedStringFromTable(@"Proceed Anyway", @"JRFeedbackProvider", nil) alternateButton:NSLocalizedStringFromTable(@"Cancel", @"JRFeedbackProvider", nil) otherButton:nil informativeTextWithFormat:NSLocalizedStringFromTable(@"Unreachable Explanation", @"JRFeedbackProvider", nil), [[JRFeedbackController postURL] host]] runModal];

        if (NSAlertDefaultReturn == alertResult) showFeedbackWindow = YES;
    }
    
    if (showFeedbackWindow) {
        if (!gFeedbackController) gFeedbackController = [[JRFeedbackController alloc] init];
        ...
        if (details) [gFeedbackController setTextViewStringTo:details];
    }
}
...
- (IBAction)switchSectionAction:(NSSegmentedControl*)sender {
    ...
    if (JRFeedbackController_SupportRequest == currentSection) [self setIncludeContactInfo:YES];
}
...
- (IBAction)submitAction:(id)sender {
    ...
    if ([includeHardwareDetailsCheckbox intValue] == 1)
        [NSThread detachNewThreadSelector:@selector(system_profilerThread:) toTarget:self withObject:nil];
    else
...
- (void)system_profilerThread:(id)ignored {
    @autoreleasepool {
        NSString *systemProfile = nil;
        {
            ...
            systemProfile = [[NSString alloc] initWithData:[[outputPipe fileHandleForReading] readDataToEndOfFile] encoding:NSUTF8StringEncoding];
        }
        [self performSelectorOnMainThread:@selector(postFeedback:) withObject:systemProfile waitUntilDone:NO];
    }
}

- (void)postFeedback:(NSString*)systemProfile {

    NSMutableDictionary *form = [NSMutableDictionary dictionaryWithObjectsAndKeys: JRFeedbackType[currentSection], @"feedbackType", [sectionStrings[currentSection] string], @"feedback", [[[NSBundle bundleForClass:[self class]] infoDictionary] objectForKey:@"CFBundleName"], @"appName", [[[NSBundle bundleForClass:[self class]] infoDictionary] objectForKey:@"CFBundleIdentifier"], @"bundleID", [[[NSBundle bundleForClass:[self class]] infoDictionary] objectForKey:@"CFBundleVersion"], @"version", nil];

    if (systemProfile) [form setObject:systemProfile forKey:@"systemProfile"];

    if ([self includeContactInfo]) {
        if ([[emailAddressComboBox stringValue] length]) [form setObject:[emailAddressComboBox stringValue] forKey:@"email"];
        if ([[nameTextField stringValue] length]) [form setObject:[nameTextField stringValue] forKey:@"name"];
    }
...
- (void)connectionDidFinishLoading:(NSURLConnection*)connection {
    ...
    [GrowlApplicationBridge notifyWithTitle:@"Thank you!" description:@"Your feedback has been sent." notificationName:@"Feedback Sent" iconData:nil priority:0 isSticky:NO clickContext:nil];
    [self closeFeedback];
#else
    //	drop thank you sheet
    [self displayAlertMessage:@"Thank you for your feedback!" withInformativeText:@"Your feedback has been sent." andAlertStyle:NSInformationalAlertStyle];
#endif
}
...
- (void)displayAlertMessage:(NSString *)message withInformativeText:(NSString *)text andAlertStyle:(NSAlertStyle)alertStyle {
    ...
    //	disply thank you
    [thankYouAlert beginSheetModalForWindow:[gFeedbackController window] modalDelegate:self didEndSelector:@selector(alertDidEnd:returnCode:contextInfo:) contextInfo:nil];
}
...
- (void)connection:(NSURLConnection*)connection didFailWithError:(NSError*)error {
    ...
    //	drop fail sheet
    [self displayAlertMessage:@"An Error Occured" withInformativeText:@"There was a problem sending your feedback.  Please try again at another time" andAlertStyle:NSInformationalAlertStyle];
}
